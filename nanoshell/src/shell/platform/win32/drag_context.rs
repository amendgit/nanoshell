...
use super::{
    ...
    drag_util::{
        ...
        create_dragimage_bitmap, CLSID_DragDropHelper, DoDragDrop, RegisterDragDrop, DROPEFFECT,
        DROPEFFECT_NONE,
    },
...
impl DragContext {
    ...
    pub fn assign_weak_self(&self, weak_self: Weak<DragContext>) {
        ...
        unsafe {
            RegisterDragDrop(window.hwnd(), get_raw_ptr(&target)).unwrap();
        }
    }
    ...
    unsafe fn start_drag_internal(&self, request: DragRequest) {
        ...
        if let Some(delegate) = window.delegate() {
            ...
            if res.0 == DRAGDROP_S_DROP as u32 {
