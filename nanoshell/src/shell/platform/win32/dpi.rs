    ...
    unsafe {
        ...
        if module != 0 {
            let function = GetProcAddress(
                module,
                const_cstr!("SetProcessDpiAwarenessContext").as_ptr(),
            );
            if let Some(set_awareness_context) = function {
                let function: extern "system" fn(isize) -> ::windows::BOOL =
...
fn set_per_monitor_dpi_aware_fallback() -> bool {
    ...
    let mut res = false;
    unsafe {
        let module = LoadLibraryW(utf16_null!("Shcore.dll").as_ptr());
        if module != 0 {
            let function = GetProcAddress(module, const_cstr!("SetProcessDpiAwareness").as_ptr());
            if let Some(set_awareness) = function {
                let function: extern "system" fn(i32) -> ::windows::BOOL =
